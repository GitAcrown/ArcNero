...
class Snowfest(commands.Cog):
    ...
    def initialize_database(self):
        for guild in self.bot.guilds:
            conn = get_sqlite_database('snowfest', 'g' + str(guild.id))
            cursor = conn.cursor()
            cursor.execute("CREATE TABLE IF NOT EXISTS members (member_id INTEGER PRIMARY KEY, score INTEGER)")
            cursor.execute("CREATE TABLE IF NOT EXISTS members_items (member_id INTEGER PRIMARY KEY, item_id TEXT, amount INTEGER CHECK (amount > 0), FOREIGN KEY (member_id) REFERENCES members(member_id))")
        conn.commit()
        cursor.close()
        conn.close()
        
    def get_inventory(self, member: discord.Member):
        conn = get_sqlite_database('snowfest', 'g' + str(member.guild.id))
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM members_items WHERE member_id=?", (member.id,))
    ...
    def get_snowball_inventory(self, member: discord.Member):
        ...
        Data = Query()
        return table.get(Data.name == 'snowballs') if table.get(Data.name == 'snowballs') else {i: 0 for i in SNOWBALLS}
    ...
    async def autocomplete_callback(self, interaction: discord.Interaction, current: str):
        ...
        return [app_commands.Choice(name=f"{SNOWBALLS[t]['name']} (x{userinv.get(t, 0)})", value=t) for t in SNOWBALLS] 
    ...
    async def make_snowball(self, interaction: discord.Interaction):
        ...
        snowballs_random = random.choices(list(SNOWBALLS.keys()), [i['make_weight'] for i in SNOWBALLS], k=nbsb)
